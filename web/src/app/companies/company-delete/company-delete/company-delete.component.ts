...
export class CompanyDeleteComponent implements OnInit {
  ...
  isPermanent=false;
  constructor(private activatedRoute: ActivatedRoute, private router: Router, private service: CompanyDeleteService) { 
  ...
  getCompanyByGuid(guid:string):void{
    this.service.getByGuid(guid).subscribe(data=>{
      if(data.isSuccess){
  ...
  delete():void{
    this.service.delete(this.company.guid, this.isPermanent).subscribe(data=>{
